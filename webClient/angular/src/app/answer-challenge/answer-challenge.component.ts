...
export class AnswerChallengeComponent implements OnInit, OnDestroy, AfterContentInit {
  ...
  ngOnInit() {
    ...
    // this.sms_.next(this.cognitoUser.ChallengeParameters);
  ...
  public async submit() {
    try {
      ...
      const answer = [1, 2, 3, 4, 5, 6]
        ...
        .join('');
      const { loginSucceeded, cognitoAnswer } = await this.auth.answerCustomChallenge(this.cognitoUser.Session, answer, this.username);
      if (loginSucceeded) {
        this.dataService.changeCognitoUser(cognitoAnswer);
        this.router.navigate(['/private']);
      } else {
        this.cognitoUser = cognitoAnswer;
